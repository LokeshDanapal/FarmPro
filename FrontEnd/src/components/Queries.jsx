...
function Queries() {
  ...
  async function handleClick(e) {
    ...
    setItem({ ...item, username: localStorage.getItem("user") });
    console.log(item);
    ...
    setItem(async (item) => {
      try {
        await Axios.post("http://localhost:5000/api1/postquery", item)
          ...
          .catch(() => {
            alert("Failed to Post the Query");
          });
      } catch (err) {
    ...
    <div className="flex justify-center">
      <div className="bg-slate-200 rounded-xl p-5 my-14 w-[40%] ">
        ...
        <div className="w-[90%] mx-auto  p-4">
          <label
            htmlFor="message"
            className="block mt-6 text-gray-700 text-lg font-bold mb-2"
          >
            Your Message:
          </label>
          <textarea
        ...
        <div className="flex items-center justify-center mt-6 gap-x-5">
          ...
          >
            ...
            <span className="text-gray-700 text-lg">
              Allow Others to View this File
            </span>
          </label>
        </div>

        <div className="bg-blue-200 p-4 mt-6 rounded-lg pl-9">
          <p className="text-black font-bold">Note Info :</p>
          <p className="text-blue-700 ">
            You can also upload pictures of your crops, plant diseases, pests,
            weeds, and more below.
          </p>
          <div className="flex justify-center mt-5">
            <FileBase64
              type="file"
              name="image"
              className="sm:ml-4 w-96 "
              multiple={false}
        ...
        <div className="flex justify-center mt-10 pb-4">
          <button
            onClick={handleClick}
            className="bg-blue-500 text-xl  hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-full shadow-md"
          >
            POST
          </button>
        </div>
